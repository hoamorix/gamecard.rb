...
loop do
break if user_desk.size == 0

puts "Введите любое число от 0 до #{user_desk.size - 1}"if user_desk.size > 1
...
sleep (rand(1..2))
puts "Ваша карта #{user_step[0]} победила. У Компьютера #{bot_step[0]}" if user_step[1] > bot_step[1]

user_score += 1 if user_step[1] > bot_step[1]

puts "Ваша карта #{user_step[0]} проиграла. У Компьютера #{bot_step[0]}" if user_step[1] < bot_step[1]

bot_score += 1 if user_step[1] < bot_step[1]

puts "Ого вот это удача #{user_step[0]}" if user_step[1] == bot_step[1]
sleep 1
puts user_score > bot_score ? "#{username} Вы победили" : "Компьютер проиграл" if user_desk.size == 0
end
